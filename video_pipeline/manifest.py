...
class RunPaths:
    ...
    def audio_path(self, paragraph_index: int, suffix: str = ".wav") -> Path:
        return self.audio_dir / f"wav_{paragraph_index:03d}{suffix}"
...
def save_manifest(paths: RunPaths, manifest: RunManifest) -> None:
    ensure_run_dirs(paths)
    paths.manifest_path.write_text(json.dumps(manifest.model_dump(mode="json"), indent=2), encoding="utf-8")
