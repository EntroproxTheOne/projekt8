...
def generate_assets(base_output_dir: Path, run_id: str, services: PipelineServices, output_mode: str = "video") -> object:
    if output_mode not in {"video", "image", "audio"}:
        raise ValueError("Unsupported output mode")
    paths = RunPaths(base_output_dir, run_id)
    manifest = create_or_load_manifest(paths, idea="resume", duration=1, audio_provider="deepgram")
    plan = load_plan(paths)
    scenes = plan.scenes[:2]
    for scene in (scenes if output_mode != "audio" else []):
        ...
        try:
            ...
            raise
        save_manifest(paths, manifest)
    for scene in (scenes if output_mode != "image" else []):
        status = manifest.scene_status(scene.paragraph_index)
        audio_path = paths.audio_path(scene.paragraph_index)
        timestamps_path = paths.timestamps_path(scene.paragraph_index)
        result = None
        try:
            if not audio_path.exists() and not audio_path.with_suffix(".mp3").exists() or not timestamps_path.exists():
            ...
            existing_audio = result.audio_path if result else (audio_path if audio_path.exists() else audio_path.with_suffix(".mp3"))
            status.audio_path = existing_audio.relative_to(paths.run_dir).as_posix()
